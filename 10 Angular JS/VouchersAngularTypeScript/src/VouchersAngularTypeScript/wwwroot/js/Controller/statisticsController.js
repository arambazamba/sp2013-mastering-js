var voucherAppTS;
(function (voucherAppTS) {
    var Controllers;
    (function (Controllers) {
        var StatisticsController = (function () {
            function StatisticsController($scope) {
                this.scope = $scope;
                this.scope.StatisticsData = [
                    { y: '2006', a: 50, b: 90 },
                    { y: '2007', a: 65, b: 75 },
                    { y: '2008', a: 50, b: 50 },
                    { y: '2009', a: 75, b: 60 },
                    { y: '2010', a: 80, b: 65 },
                    { y: '2011', a: 90, b: 70 },
                    { y: '2012', a: 100, b: 75 },
                    { y: '2013', a: 115, b: 75 },
                    { y: '2014', a: 120, b: 85 },
                    { y: '2015', a: 145, b: 85 },
                    { y: '2016', a: 160, b: 95 }
                ];
            }
            return StatisticsController;
        }());
        Controllers.StatisticsController = StatisticsController;
        voucherAppTS.voucherApp.controller("statisticsController", StatisticsController);
    })(Controllers = voucherAppTS.Controllers || (voucherAppTS.Controllers = {}));
})(voucherAppTS || (voucherAppTS = {}));
//# sourceMappingURL=statisticsController.js.map